...
func (s *PaymentService) processPaid(c *gin.Context, dep dependency.Dep, notify verifiedNotify) error {
	...
	if err := fulfillOrder(ctx, dep, txUser, txEvent, order, notify.Provider); err != nil {
...
func fulfillOrder(ctx context.Context, dep dependency.Dep, userClient inventory.UserClient, eventClient inventory.EventClient, order *ent.Order, provider types.PaymentProvider) error {
	product, err := dep.ProductClient().GetByID(ctx, order.ProductID)
	if err != nil {
