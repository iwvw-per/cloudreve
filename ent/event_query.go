...
func (eq *EventQuery) QueryEntity() *EntityQuery {
...
func (eq *EventQuery) QueryShare() *ShareQuery {
	query := (&ShareClient{config: eq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eq *EventQuery) WithEntity(opts ...func(*EntityQuery)) *EventQuery {
	query := (&EntityClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) WithShare(opts ...func(*ShareQuery)) *EventQuery {
	query := (&ShareClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (eq *EventQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Event, error) {
	...
	if query := eq.withEntity; query != nil {
		if err := eq.loadEntity(ctx, query, nodes, nil,
			func(n *Event, e *Entity) { n.Edges.Entity = e }); err != nil {
			return nil, err
		}
	}
	if query := eq.withShare; query != nil {
		if err := eq.loadShare(ctx, query, nodes, nil,
			func(n *Event, e *Share) { n.Edges.Share = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (eq *EventQuery) loadEntity(ctx context.Context, query *EntityQuery, nodes []*Event, init func(*Event), assign func(*Event, *Entity)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		fk := nodes[i].EntityEvents
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "entity_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EventQuery) loadShare(ctx context.Context, query *ShareQuery, nodes []*Event, init func(*Event), assign func(*Event, *Share)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Event)
	for i := range nodes {
		fk := nodes[i].ShareEvents
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "share_events" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (eq *EventQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := eq.ctx.Fields; len(fields) > 0 {
		...
		if eq.withUser != nil {
			_spec.Node.AddColumnOnce(event.FieldUserEvents)
		}
		if eq.withFile != nil {
			_spec.Node.AddColumnOnce(event.FieldFileEvents)
		}
		if eq.withEntity != nil {
			_spec.Node.AddColumnOnce(event.FieldEntityEvents)
		}
		if eq.withShare != nil {
			_spec.Node.AddColumnOnce(event.FieldShareEvents)
		}
	}
